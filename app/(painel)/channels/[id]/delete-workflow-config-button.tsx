...
interface DeleteWorkflowConfigButtonProps {
  id: number;
  name: string;
}

export function DeleteWorkflowConfigButton({ id, name }: DeleteWorkflowConfigButtonProps) {
  const { open, setOpen, pending, error, handleOpenChange, handleSubmit } = useDialogForm();

  function handleDelete() {
    return handleSubmit(
      new FormData(),
      () => fetch(`/api/workflow-configs/${id}`, { method: "DELETE" }),
